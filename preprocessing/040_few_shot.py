...
def main(args):
    ...
    if args.color == 'green':
        ...
        try:
            ...
            os.makedirs(str(FewShot) + '/images_{}'.format(args.few_shot), exist_ok=True)
        except:
            pass
        os.makedirs(str(AugCropImagesFewShot) + '_{}'.format(args.few_shot), exist_ok=True)
        os.makedirs(str(FewShot) + '/images_{}'.format(args.few_shot), exist_ok=True)
        full_images_path = str(FewShot) + '/images_{}'.format(args.few_shot)
        try:
            shutil.rmtree(str(AugCropMasksFewShot) + '_{}'.format(args.few_shot))
            os.makedirs(str(FewShot) + '/masks_{}'.format(args.few_shot), exist_ok=True)
        except:
            pass
        os.makedirs(str(AugCropMasksFewShot) + '_{}'.format(args.few_shot), exist_ok=True)
        os.makedirs(str(FewShot) + '/masks_{}'.format(args.few_shot), exist_ok=True)
        full_masks_path = str(FewShot) + '/masks_{}'.format(args.few_shot)
        ...
        for im_names in cropped_names:
            ...
            if name in to_sample_root:
                ...
                shutil.copy(os.path.join(aug_cropped_train_val_images,im_names), os.path.join(str(AugCropImagesFewShot)\
                                                                                              + '_{}'.format(args.few_shot), im_names))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--few_shot', nargs="?", default=70, help='the folder including the masks to crop')
